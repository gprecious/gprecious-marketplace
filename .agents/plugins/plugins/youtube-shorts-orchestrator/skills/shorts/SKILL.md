...
이 skill은 OpenCode 플러그인에서 이식되었다. Codex에서 사용할 때는 `todowrite`를 `update_plan`으로, `background_task` 병렬 호출은 사용 가능한 subagent/멀티-pane 도구나 직접 병렬 작업으로 대응한다. `agents/`, `channels/`, `content-bank/`, `history/`, `scripts/`, `wisdom/` 자료는 이 Codex plugin root에 함께 포함되어 있으므로 필요한 파일을 상대 경로로 읽어 진행한다.
