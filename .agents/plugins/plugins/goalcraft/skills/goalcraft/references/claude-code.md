...
- **`/goal <condition>`** — for long-horizon, mostly-unattended work. Sets a session-scoped completion condition that a fast model re-checks after every turn until it holds. Use it when the task is a sustained grind (a migration, an optimization loop) rather than a single change. Make the condition measurable and bound it. Example:
  ...
  >
  > **The evaluator judges only what's in the conversation.** That fast re-checking model cannot run commands or read files on its own — it reads the transcript. So phrase the condition against something the agent's *own output* demonstrates, and tell the prompt to print the proof each turn (the test summary, the exit code, the count reaching zero). `"npm test exits 0"` works only because the agent runs it and the result lands in the transcript for the evaluator to read; a condition about hidden file state that never gets printed either never clears or clears on a vibe.
  >
