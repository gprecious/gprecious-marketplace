...
## 2026-06-10 — NeonNovel Android 첫 심사 제출 (Play 비공개 테스트 경유)

### L5. `[일반]` 개인(2023-11 이후 생성) Play 계정은 프로덕션 직행 불가 — 첫 심사는 비공개 테스트 게시로 받는다
- 프로덕션 액세스 신청 요건: **12명 이상 테스터가 14일 이상 비공개 테스트 참여** + 신청 시 질문 답변. 내부 테스트는 검토(심사)를 트리거하지 않는다.
- 따라서 "심사 등록"의 최단 경로 = 비공개 테스트(Alpha) 트랙: 국가 선택 → 테스터 목록 선택 → 버전 생성(라이브러리에서 기존 AAB 재사용 가능) → 저장 → **게시 개요(Publishing overview)에서 "검토를 위해 변경사항 전송"**. 전송 버튼은 트랙 페이지가 아니라 게시 개요에 있다.
- 전송 직전 "일반적으로 발견되는 문제에 대한 빠른 검사"(최대 14분)가 돌고, 전송을 누르면 검사 성공 시 자동으로 검토에 들어간다.
- 비공개 테스트 게시에는 테스터 수 요건이 없다(1명 목록으로도 제출 가능). 12명×14일은 프로덕션 신청 단계의 요건.

### L6. `[일반]` 스토어 등록정보는 fastlane supply로 전부 자동화 가능, 단 카테고리·연락처는 콘솔 UI 전용
- `upload_to_play_store`(서비스 계정 키)로 title/short/full description + icon(512) + featureGraphic(1024×500) + phoneScreenshots 업로드 가능. `sync_image_upload: true`면 sha256 비교로 멱등.
